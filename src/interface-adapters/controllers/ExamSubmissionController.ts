...
export class ExamSubmissionController {
  ...
  start = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const session = await this.manageExamSessions.startExam(
        data.userId,
        data.examId,
        data.timezone,
      );
      ...
      if (
        session.currentSectionSession &&
        session.currentSectionSession.endTimeLimit &&
        session.currentSectionSession.timezone
      ) {
        try {
          endTimeLocale = toLocalISOString(
            new Date(session.currentSectionSession.endTimeLimit),
            session.currentSectionSession.timezone,
          );
        } catch (e) {
      ...
      if (error.message === "Ongoing session already exists") {
        ...
        if (
          error.session &&
          error.session.currentSectionSession &&
          error.session.currentSectionSession.endTimeLimit &&
          error.session.timezone
        ) {
          try {
            endTimeLocale = toLocalISOString(
              new Date(error.session.currentSectionSession.endTimeLimit),
              error.session.timezone,
            );
          } catch (e) {}
        }
  ...
  finish = async (
    ...
    try {
      ...
      const result = await this.manageExamSessions.finishExam(
        req.params.id,
        data.timezone,
      );
  ...
  finishSection = async (
    ...
    try {
      const data = finishSchema.parse(req.body || {});
      const nextSectionSubmission = await this.manageExamSessions.finishSection(
        req.params.id,
        data.timezone,
      );
      let endTimeLocale: string | undefined;
      if (
        nextSectionSubmission &&
        nextSectionSubmission.endTimeLimit &&
        nextSectionSubmission.timezone
      ) {
        try {
          endTimeLocale = toLocalISOString(
            new Date(nextSectionSubmission.endTimeLimit),
            nextSectionSubmission.timezone,
          );
        } catch (e) {
          // Ignore if valid timezone is not available
      ...
      res
        .status(200)
        .json({
          message: "Section finished",
          nextSectionSubmission: { ...nextSectionSubmission, endTimeLocale },
        });
    } catch (error: any) {
      if (error.message === "Time window exceeded") {
        return res.status(400).json({ message: error.message });
  ...
  recordAnswer = async (
    ...
    try {
      ...
      if (
        error.message === "Time window exceeded" ||
        error.message === "Section is not currently ongoing"
      ) {
        return res.status(400).json({ message: error.message });
      }
  ...
  getHistory = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const history =
        await this.manageExamSessions.getSubmissionHistoryByUserId(
          authReq.user.userId,
        );

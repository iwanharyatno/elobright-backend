...
const createQuestionSchema = z.object({
  ...
  ), // Handles formData numeric strings
});
...
export class QuestionController {
  ...
  create = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const question = await this.manageQuestions.create(
        data as any,
        audioFile,
        questionAudioFile,
        imageFile,
  ...
  update = async (
    ...
    try {
      ...
      const question = await this.manageQuestions.update(
        req.params.id,
        data as any,
        audioFile,
        questionAudioFile,
  ...
  reorder = async (
    ...
    try {
      ...
      if (direction !== "up" && direction !== "down") {
        return res
          .status(400)
          .json({ message: "Invalid direction. Must be 'up' or 'down'." });
      }
      const success = await this.manageQuestions.reorder(
        req.params.id,
        direction,
      );
      if (!success) {
        return res
          .status(404)
          .json({
            message: "Question not found or cannot be moved in that direction",
          });
      }
